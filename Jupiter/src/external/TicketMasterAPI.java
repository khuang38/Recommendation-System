...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "3jZ4IQGfqZjADJ6ZLgEyWBYUhuqDoNAs";
	
	public List<Item> search(double lat, double lon, String keyword) {
		//If the client does not specify the keyword, then use the DEFAULT_KEYWORD
		if (keyword == null) {
		...
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8"); //Ticketmaster need UTF-8 need Use .encode to change blank space to %"Rick Sun" => "Rick%20Sun"
		...
		try {
			...
			if (!obj.isNull("_embedded")) {//the information we need is under key "_embedded" 
		...
		return new ArrayList<>(); //no results returned from ticketmaster api
	}
